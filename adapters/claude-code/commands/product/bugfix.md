...
7. As soon as the bug issue is chosen, initialize the quick-run state:
   - `bash ~/Documents/Dev/gulyaev-forge/scripts/forge-run-state.sh begin-bugfix . <issue-number> "<issue-title>"`
8. Default path:
   - `implementation`
   - `test_coverage`
   - `qa`
...
11. Record the Context7 decision in quick-run state:
   - `bash ~/Documents/Dev/gulyaev-forge/scripts/forge-run-state.sh set-context7 . yes "<reason>"`
   - or `bash ~/Documents/Dev/gulyaev-forge/scripts/forge-run-state.sh set-context7 . no "<reason>"`
...
13. Before leaving implementation, update the quick-run stage:
   - `bash ~/Documents/Dev/gulyaev-forge/scripts/forge-run-state.sh set-stage . test_coverage`
...
16. Before presenting QA, update the quick-run state:
   - `bash ~/Documents/Dev/gulyaev-forge/scripts/forge-run-state.sh set-stage . qa`
   - `bash ~/Documents/Dev/gulyaev-forge/scripts/forge-run-state.sh set-gate . pending_approval`
   - write a durable issue comment with heading `## QA Gate`
   - run `bash ~/Documents/Dev/gulyaev-forge/scripts/forge-issue-trail.sh check-bugfix-qa . <issue-number>`
