...
10. Determine communication scope:
   - if the release is user-facing, create or update a canonical packet under `docs/release-notes/`
   - treat that packet as the source of truth for user-facing wording
   - if `.forge/config.yaml` defines project-owned communication channels, adapt to them
   - otherwise prepare at least:
     - website / changelog copy
     - primary community-channel copy
     - short social variant
     - store / beta notes when relevant to the chosen target
   - make channel status explicit: `draft`, `ready`, `published`, or `n/a`
11. Start by showing a short preload summary:
   - chosen target
   - chosen candidate issue/run
   - runbook source
   - release communication artifact path, if applicable
   - whether upload will execute now or what blocks it
12. If the user explicitly asked to upload/release and the preconditions pass:
   ...
   - report which release-note channels are already published versus only prepared
...
13. If the target is not ready, stop with an explicit release gate:
   - what is ready
   - what is blocked
   - exact next action
