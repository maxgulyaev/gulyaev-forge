...
- `~/Documents/Dev/gulyaev-forge/core/pipeline/entry-surface.md`
...
Важно: запрос вроде `Прогони #95 до PRD gate` не разрешает перепрыгивать незакрытые гейты.
...
# 3. Подключи к forge
bash ~/Documents/Dev/gulyaev-forge/bin/forge init --project .
...
```bash
bash ~/Documents/Dev/gulyaev-forge/bin/forge init --project . --labels
```
...
### 1.1 Создай .forge/config.yaml и .forge/skills в Spodi

```bash
mkdir -p ~/Documents/Dev/spodi/.forge
mkdir -p ~/Documents/Dev/spodi/.forge/skills
```
...
```bash
cp ~/Documents/Dev/gulyaev-forge/core/templates/project-overlay-skill.md \
  ~/Documents/Dev/spodi/.forge/skills/implementation.md
```
...
bash ~/Documents/Dev/gulyaev-forge/scripts/forge-issue-trail.sh check-bugfix-qa . 101
```
...
- [ ] `bash ~/Documents/Dev/gulyaev-forge/scripts/forge-doctor.sh self .` проходит без ошибок
